...
HemiSpec Toolkit is organized as a deployable Python package plus approved reusable model assets. The public repository should make the runtime contract clear without committing private MRI data or generated outputs.
...
|-- assets/                               # approved model bundles plus local asset manifests
|   |-- atlases/glasser/                  # local Glasser atlas + label table, not tracked unless approved
...
- Approved reusable DGN checkpoints and classifier bundles under `assets/models/` via Git LFS.
...
4. The per-user cache populated by first-run model download.
5. Legacy root folders such as `outputs_bi_stable_L/R`, for compatibility only.

The PyPI package remains lightweight and does not embed large DGN checkpoints, classifier bundles, atlas payloads, or subject-level examples. Released DGN/classifier defaults are downloaded into the user cache when needed. Compiled app distributions can ship approved offline assets beside the application folder with a manifest and checksums.
