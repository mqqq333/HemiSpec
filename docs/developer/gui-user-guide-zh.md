...
这是推荐入口。用户提供几个被试的预处理 GM 图像后，工作流会自动运行两个模型方向：
...
```text
voxel-wise ANS.L / ANS.R / RNS.L / RNS.R
ROI-wise ANS.L / ANS.R / RNS.L / RNS.R
ANS.L_mean / ANS.R_mean / RNS.L_mean / RNS.R_mean
ANS.whole_brain_mean / RNS.whole_brain_mean
hemisphere classifier predictions and accuracy
...
```text
Preprocessed GM glob        输入 GM 文件通配符
Workflow output directory   输出目录
Bundled DGN root            默认 assets/models/dgn
Device                      auto/cpu/cuda
ROI atlas NIfTI             默认 Glasser atlas
ROI label table             默认 Glasser label table
Run hemisphere classifier   默认开启
Run TRT reliability         输入是重测数据时开启
TRT file regex              从文件名解析 subject/session
```

主要输出见 `docs/outputs_zh.md`。
