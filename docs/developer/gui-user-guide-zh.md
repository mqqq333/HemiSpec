...
2. Output workspace：输出目录。流程会创建 voxel_maps/、tables/、validation/；intermediate/ 只在勾选保留中间结果时存在。
...
标准 workflow 的主要输出是每个被试 4 个 voxel-wise ANS/RNS maps：
...
默认不保留 DGN reconstruction 和单方向中间 metrics；如果需要调试，可勾选 Keep intermediate reconstructions and one-direction metrics。
...
如果启用 classifier 或 TRT，会分别在 validation/hemi_classify/ 和 validation/trt/ 生成 validation summary。
